...
class TestMain(unittest.TestCase):
    ...
    def test_process_document(self, mock_load_prompts, mock_CSVWriter, mock_validate_client, mock_extract_client, mock_classify_client, mock_digitize_client):
        ...
        process_document("test_document.pdf", "output_directory", True, False, True, False)

        mock_digitize_client.digitize.assert_called_once_with("test_document.pdf")
        mock_classify_client.classify_document.assert_called_once_with('DOC123', 'generative_classifier', {'prompt1': 'value1'}, True)
    ...
    def test_process_documents_in_folder(self, mock_process_document, mock_listdir):
        ...
        process_documents_in_folder("test_folder", "output_directory", True, True, False, False)

        self.assertEqual(mock_process_document.call_count, 2)
        mock_process_document.assert_any_call("test_folder/doc1.pdf", "output_directory", True, True, False, False)
        mock_process_document.assert_any_call("test_folder/doc2.png", "output_directory", True, True, False, False)
